    ...
    class MoodleQuizSolver {
        ...
        extractQuestionText(element) {
            ...
            if (qtext) {
                ...
                // Случай 1: "m = 1m = 1", "m = 10m = 10" (буква = число буква = число)
                text = text.replace(/([a-zA-Zа-яА-Я]+)\s*=\s*(\d+(?:\.\d+)?)\s*([a-zA-Zа-яА-Я]+)\s*=\s*\2/g, '$1 = $2');
                
                // Случай 2: "m = 1 m = 1", "m = 10 m = 10" (с пробелом между)
                text = text.replace(/([a-zA-Zа-яА-Я]+)\s*=\s*(\d+(?:\.\d+)?)\s+([а-яА-Я]+)?\s+\1\s*=\s*\2(?:\s+\3)?/g, '$1 = $2 $3');
                
                // Случай 3: "m2=5m2=5", "ε=120ε=120" (без пробелов, с цифрами в переменной)
                text = text.replace(/([a-zA-Zа-яА-Я0-9]+)=(\d+(?:\.\d+)?)\1=\2/g, '$1=$2');
                
                // Случай 4: "s = 4t3s = 4t3" (переменная содержит цифры)
                text = text.replace(/([a-zA-Zа-яА-Я]+)\s*=\s*(\d+[a-zA-Zа-яА-Я0-9]+)\s*\1\s*=\s*\2/g, '$1 = $2');
                
                // Случай 5: "a = 14a = 14" (переменная = число переменная = число без пробела)
                text = text.replace(/([a-zA-Zа-яА-Я]+)\s*=\s*(\d+(?:\.\d+)?)([a-zA-Zа-яА-Я]+)\s*=\s*\2/g, '$1 = $2');
                
                // Применяем еще раз для случаев, которые могли остаться
